...
if phase == 'S':
  ...
  example_params='/data/dna/rmartin/ALASKA/prep_for_tomo/xcorr_input_P.txt'
else:
  print 'Input phase %s not recogized' %phase
  ssy.exit(1)
...
def cull_xcorrelations(phase):
   ...
   #If xcorrection has already been done, remove the associated files
   os.system('rm *.clean')
   ...
   for infile in xcorrfiles:
      ...
      for line in lines:
         ...
         if len(vals[-2]) < 10:
            ...
            #Criteria for selecting 'good' station/event pairs
...
for event in events:
  ...
  if results.allx:
    ...
    #This is the path we need to follow - it turns out that that cross correlation method doesn't really care about which station is chosen as reference, so we
    #can use the same cross correlation parameter file for all events
  ...
  #Check if a cross correlation param file exists
